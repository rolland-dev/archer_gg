...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($points_err) && empty($classement_err) ){

        $sql = "UPDATE inscriptions SET points=? ,classement=? WHERE id=?";

        if($stmt = mysqli_prepare($link, $sql)){
            mysqli_stmt_bind_param($stmt, "iii", $param_points, $param_classe, $param_id);
            
            $param_points = $points;
            $param_classe = $classement;
            $param_id = $_GET['id'];
